...
def insert_record(driverInfo):
    ...
    values=(driverInfo.getDid(), driverInfo.getName(), driverInfo.getMobile(), driverInfo.getEmail(),
            driverInfo.getLicense(), driverInfo.getPassword(),driverInfo.getDriverstatus(), driverInfo.getStatus(), driverInfo.getVehicletype())
    result=False
    try:
        conn=Connect()
        cursor=conn.cursor()
...
def search_record(did):
    conn=None
    sql="""SELECT *, old_password(password) as Pw FROM drivers WHERE did=?"""
    values=(did,)
    searchResult=None
    try:
        conn=Connect()
...
def update_record(driverInfo):
    ...
    values=(driverInfo.getName(), driverInfo.getMobile(), driverInfo.getEmail(), driverInfo.getLicense(), driverInfo.getVehicletype(), driverInfo.getDid())
    updateresult=False
    try:
        conn=Connect()
        cursor=conn.cursor()
